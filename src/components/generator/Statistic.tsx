...
export const Statistic = ({language, slot, statSlot, setStatSlot}: {
  language: Language,
  slot: string,
  ...
  return <div className={'setting'}>
    ...
    <select value={statSlot} onChange={(event) => {
      console.log(`${event.target.value}`);
      setStatSlot(event.target.value as StatisticType);
    }}>
      {Object.values(StatisticType).map((value) => {
        return <option key={value} value={value}>{tl(language, `stat.${value.toLowerCase()}`)}</option>
      })}
    </select>
  </div>
}