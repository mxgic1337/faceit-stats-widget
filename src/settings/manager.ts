...
export type SettingValue = string | number | boolean | StatisticType | Language | ShowRanking | undefined;

export type SettingValueType<K extends SettingKey> = typeof SETTINGS_DEFINITIONS[K]['type'] extends keyof SettingValueTypeMap ? SettingValueTypeMap[typeof SETTINGS_DEFINITIONS[K]['type']] : unknown;

export type SetSettingFunction = <K extends SettingKey>(key: K, value: SettingValueType<K>) => boolean;
...
export function useSettings(useWidgetDefaults?: boolean) {
  ...
  const setSetting = useCallback(<K extends SettingKey>(key: K, value: SettingValueType<K>): boolean => {
    const definition: SettingDefinition = SETTINGS_DEFINITIONS[key];
    if (definition.min !== undefined && definition.max !== undefined && typeof value === "number") {
    ...
    if (definition.options && !definition.options.includes(value as SettingValue)) return false;
    ...
    (Object.entries(SETTINGS_DEFINITIONS) as [SettingKey, SettingDefinition][]).forEach(([key, definition]) => {
      ...
      definition.query.forEach(query => {
        ...
        if (definition.type === 'number' || definition.type === 'ranking_state') {
          const number = definition.type === 'ranking_state' ? parseInt(queryVal) : parseFloat(queryVal);
          if (!isNaN(number)) {
  ...
  const getSetting = useCallback(<K extends SettingKey>(key: K | string): SettingValueType<K> => {
